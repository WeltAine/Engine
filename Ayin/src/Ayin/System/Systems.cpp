...
namespace Ayin{
	...
	void RenderSystem::OnUpdate(const SystemContext& systemContext) {
		...
		auto findMainCamera = [&max_RenderOrder, &mainCamera](::entt::entity, CameraComponent& cameraComponent) -> void {
			
			max_RenderOrder = std::max(max_RenderOrder, cameraComponent.RenderOrder);

			if (max_RenderOrder == cameraComponent.RenderOrder)
				mainCamera = &cameraComponent.Camera;

			};

		Camera* sortCamera = nullptr;
		Camera* renderCamera = nullptr;

		systemContext.Scene.Each<CameraComponent>(findMainCamera);

		switch (systemContext.Mode) {
		
		case SceneMode::Editor:
			sortCamera = systemContext.EditorView;
			renderCamera = systemContext.EditorView;
			break;
		case SceneMode::Simulation:
			sortCamera = mainCamera;
			renderCamera = systemContext.EditorView;
			break;
		case SceneMode::Runtime:
			sortCamera = mainCamera;
		...
		// 渲染更新
		{
			...
			std::ranges::sort(renderEntities, [this, &sortCamera, &systemContext](const ::entt::entity& entity_1, const ::entt::entity& entity_2) -> bool
				{
					glm::mat4 viewMatrix = sortCamera->GetViewMatrix();
